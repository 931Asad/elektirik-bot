    ...
    with closing(get_conn()) as conn, conn:
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS media (
                ...
                media_type TEXT NOT NULL DEFAULT 'photo'   -- 'photo' | 'video' | 'video_note'
            )
        """)
        conn.execute("""
...
def add_media(object_id: int, file_id: str, media_type: str = "photo"):
    with closing(get_conn()) as conn, conn:
        conn.execute(
            "INSERT INTO media (object_id, file_id, media_type) VALUES (?, ?, ?)",
            (object_id, file_id, media_type),
        )
...
def get_media(object_id: int):
    with closing(get_conn()) as conn:
        return conn.execute(
            "SELECT file_id, media_type FROM media WHERE object_id = ? ORDER BY id", (object_id,)
        ).fetchall()


# ---------- ISH JURNALI (work log) ----------

def add_log(object_id: int, note: str, author_name: str, created_by: int):
    with closing(get_conn()) as conn, conn:
        conn.execute(
            "INSERT INTO work_logs (object_id, note, author_name, created_by) VALUES (?, ?, ?, ?)",
            (object_id, note, author_name, created_by),
        )


def get_logs(object_id: int, limit: int = 5):
    with closing(get_conn()) as conn:
        return conn.execute(
            "SELECT note, author_name, created_at FROM work_logs "
            "WHERE object_id = ? ORDER BY id DESC LIMIT ?",
            (object_id, limit),
        ).fetchall()
